...
def callbackFunctionVid(imgMatrix):
	...
	try:
		...
		if final_sign:  # non-empty
			...
			accuracy = np.amax(prediction)
			if (accuracy < 0.3):
				cv2.putText(img, "Now,I see: nothing !!", org=Text.FIRST_LINE_ORG, fontFace=cv2.FONT_HERSHEY_SIMPLEX, fontScale=Text.FONT_SIZE, thickness=1, color=(0, 255, 0))
				cv2.imshow('final', imgMatrix)
				return
			print("4")
			text = "Now,I see: " + labelToText[np.argmax(prediction)] + " with " + str(accuracy) + "%"
			...
			cv2.putText(img, "nothing !!", org=Text.FIRST_LINE_ORG, fontFace=cv2.FONT_HERSHEY_SIMPLEX, fontScale=Text.FONT_SIZE, thickness=Text.THICKNESS, color=(0, 255, 0))
			cv2.imshow('final', imgMatrix)
