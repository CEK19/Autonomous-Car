...
dstPos = np.float32([(16+275,2+275),(38+275,2+275),(19+275,-21+275),(37+275,-21+275)])
transformMatrix = cv2.getPerspectiveTransform(srcPos, dstPos)

srcPos = np.float32([(int(13*5/2),int(71*15/8)),(int(113*5/2),int(76*15/8)),(int(37*5/2),int(42*15/8)),(int(91*5/2),int(43*15/8))])
dstPos = np.float32([(16+275,2+275),(38+275,2+275),(19+275,-21+275),(37+275,-21+275)])
transformImageMatrix = cv2.getPerspectiveTransform(srcPos, dstPos)

initTime = time.time()

files = os.listdir("D:/NDT/PY_ws/DCLV/ROS_zone/newBackend_1/image1")

files.sort(key=len)

for imageName in files:
    
    print("Run frame ",imageName)
    # print("recive time:",time.time()-initTime)
    orgFrame = cv2.imread("D:/NDT/PY_ws/DCLV/ROS_zone/newBackend_1/image1/"+imageName)
    frame = cv2.cvtColor(orgFrame,cv2.COLOR_RGB2GRAY)
    frame = cv2.resize(frame,(128,128))
    # if counter%2 == 0:
    # cv2.imwrite("./his/ras4-"+str(counter)+".png",frame)
    ...
    visualMap = np.zeros((600,600,3),dtype='uint8')
    ...
    if len(listPoint[0]) + len(listPoint[1]) == 4:
        ...
        visualMap2 = cv2.warpPerspective(orgFrame,transformImageMatrix,(600,600))
    ...
    cv2.imshow("logData",visualMap2)
    cv2.imwrite("./his1/"+imageName,visualMap2)
