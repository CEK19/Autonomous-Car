    ...
    if ret == True:
        ...
        if len(leftLaneData) != 0 and len(rightLaneData) != 0:
            ...
            for eachLane in [leftLaneData,rightLaneData]:
                ...
                x0 = a * rho
                y0 = b * rho
                pt1 = (int(x0 + 1000*(-b)+128), int(y0 + 1000*(a)))
                pt2 = (int(x0 - 1000*(-b)+128), int(y0 - 1000*(a)))
        ...
        # =============== Show output ======================
        counter += 1
        print("frame",counter,"done in",avgTime*1000,"ms")
        cv2.imshow("inputFrame",frame)
        cv2.imshow("outputImage",outputImage)
        # cv2.imwrite(outputPath+"toRL.png",frame)
        if 27 == cv2.waitKey(1):
            break
