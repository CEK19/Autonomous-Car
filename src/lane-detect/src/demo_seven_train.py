...
class stupidModel:
    def unet(self, input_size, loss, metrics):
        ...
        self.model.compile(optimizer=Adam(lr=1e-3), loss=loss, metrics=['accuracy', tf.keras.metrics.Precision(), tf.keras.metrics.Recall()])
...
workingDir  = "D:/container/AI_DCLV/1-5"
testDir = "D:/container/AI_DCLV/readData/test_v1"
dataLink = "D:/container/AI_DCLV/readData/labeled_v7"
backgroundLink = "D:/container/AI_DCLV/readData/background_img"

# cnn = load_model(workingDir+"/Model-S3_AUTO-"+str(len(os.listdir(workingDir)))+".hdf5",custom_objects={'focal_tversky':focal_tversky, 'dice':dice})
...
print("See",len(dataX),"image train")
...
X_train, X_test, y_train, y_test = train_test_split(listDataX, listDataY, test_size=0.1)

history = cnn.fit(X_train,y_train, batch_size =4, epochs = 16,validation_data=(X_test,y_test))
f = open("D:/NDT/PY_ws/DCLV/src/log_autoTrain.txt", "w")
