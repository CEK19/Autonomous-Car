...
pathTest = "D:/NDT/PY_ws/DCLV/ROS_zone/history/"
pathOutput = "D:/NDT/PY_ws/DCLV/ROS_zone/ans/"
...
for eachVid in os.listdir(pathTest):
    dataX = []
    dataY = []
    nameList = os.listdir(pathTest)
    cap = cv2.VideoCapture(pathTest+"/"+eachVid)
    orgImgList = []
    while(cap.isOpened()):
    # Capture frame-by-frame
        ...
        if ret == True:
            ...
            orgImgList.append(frame.copy())
            tmpX = cv2.cvtColor(frame,cv2.COLOR_RGB2GRAY)
    ...
    out = cv2.VideoWriter(pathOutput+eachVid,cv2.VideoWriter_fourcc('M','J','P','G'), 30, (128,128))
