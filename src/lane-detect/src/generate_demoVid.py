    ...
    for i in range(img_a.shape[0]):
        for j in range(img_a.shape[1]):
            if img_b[i, j] < 0.5:
                ...
                img_c[i, j][0] /= 2
                img_c[i, j][1] /= 2
                img_c[i, j][2] = min (255,img_c[i, j][2]/2 + 255)
...
dataLink = "D:/NDT/PY_ws/DCLV/ROS_zone"

nameList = os.listdir(dataLink+"/his3")[1:]

nameList.sort(key=len)

for each in nameList:
# Capture frame-by-frame
    frame = cv2.imread(dataLink+"/his3/"+each)
...
for each in range(0,len(dataX)):
    ...
    tmp = tmp*255
    orgImg = cv2.imread(dataLink+"/his4/"+nameList[each].replace("ras4-","f"))
    tmp = cv2.resize(tmp,(orgImg.shape[1],orgImg.shape[0]))
    cv2.imwrite(dataLink+"/image/"+nameList[each],tmp)
    ...
    outImg = cv2.addWeighted(tmp,0.7,orgImg,1,0)
    # cv2.imshow("predic",outImg)
    cv2.imwrite(dataLink+"/predic/"+nameList[each],outImg)
