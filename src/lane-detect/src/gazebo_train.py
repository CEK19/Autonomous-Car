...
# cnn = load_model("D:/container/AI_DCLV/27-11/Model-CARLA10_R_epoch1.hdf5",custom_objects={'focal_tversky':focal_tversky, 'dice':dice})


dataX = []
dataY = []

nameList = os.listdir("D:/container/AI_DCLV/readData/labeled_v4/image/")

print("Start read data")

for eachName in nameList:
    # print(eachName)
    tmpX = cv2.imread("D:/container/AI_DCLV/readData/labeled_v4/image/" + eachName)
    ...
    # tmpX = ImageArgument(tmpX,5,10,3)
    # cv2.imshow("--",tmpX)
    # cv2.waitKey(500)

    tmpY = cv2.imread("D:/container/AI_DCLV/readData/labeled_v4//label/" + eachName)
...
org_dataX  = np.array(dataX)
org_dataY  = np.array(dataY)
...
for i in range(0,16):
    ...
    dataY = []
    for j in range(256):
        idx = np.random.randint(0,len(org_dataX))
        dataX.append(ImageArgument(org_dataX[idx],int(random.random()*40 - 20),random.random()*50,int(random.random()*3)+1))
        dataY.append(org_dataY[idx])
    dataX = np.array(dataX,dtype="float32")
    dataY = np.array(dataY,dtype="float32")
    
    dataY /= 255
    print(" Start train !")
    history = cnn.fit(dataX, dataY, batch_size = 4, epochs = 1)
    # cnn.save("D:/container/AI_DCLV/last/Model-CARLA10_last.hdf5")
    # if i%5 == 0:
    cnn.save("D:/container/AI_DCLV/gazebo/Model-CARLA11_GAZEBO_4-2-e"+str(i)+".hdf5")
